...
func main() {
  ...
  for _, line := range input {
    digit, err := grabCalibrationValueFromLine(line); if err != nil {
      digit = 0
    }
    sum += digit
  }
...
func grabCalibrationValueFromLine(line string) (int, error) {
  ...
  for index := 0; index < len(line)-1; index++ {
    ...
    _, err := strconv.Atoi(string(line[index])); if err == nil && firstDigit == "" {
      firstDigit = string(line[index])
    }

    _, err2 := strconv.Atoi(string(line[j])); if err2 == nil && lastDigit == "" {
